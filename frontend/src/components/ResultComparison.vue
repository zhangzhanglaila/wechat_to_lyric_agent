  ...
  <div class="bg-white rounded-2xl shadow-card p-5 border border-border-light">
    <div class="flex items-center gap-2 mb-4">
      ...
      <span class="text-xs font-semibold text-text-secondary uppercase tracking-widest">
        {{ hasBaseline ? 'Baseline vs Optimized' : 'Generated Result' }}
      </span>
      <span
        v-if="hasBaseline"
    ...
    <div class="grid grid-cols-1 gap-4" :class="hasBaseline ? 'md:grid-cols-2' : ''">

      <!-- Baseline -->
      <div v-if="hasBaseline" class="border border-border-light rounded-xl overflow-hidden">
      ...
      <div class="border border-bilibili-pink/30 rounded-xl overflow-hidden bg-bilibili-pink/5">
        <div class="px-4 py-2.5 border-b border-bilibili-pink/20 flex items-center justify-between bg-bilibili-pink/5">
          <div class="flex items-center gap-2">
            <span class="text-xs text-bilibili-pink font-medium">{{ hasBaseline ? 'Optimized' : 'Output' }}</span>
            <span class="text-xs text-text-tertiary">({{ result.path || 'advanced' }})</span>
          </div>
          <div class="flex items-center gap-2">
    ...
    <!-- Score Details -->
    <div v-if="hasBaseline" class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
...
const hasBaseline = computed(() => Boolean(props.result?.baseline_text))
